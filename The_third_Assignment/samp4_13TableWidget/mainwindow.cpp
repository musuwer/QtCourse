...
// 假设你有一个方法来解析 PDF，并返回学生名单
// 这里是一个模拟数据，实际上你可以从 PDF 中提取学号和姓名信息
QList<QStringList> extractStudentDataFromPDF(const QString &pdfFilePath)
{
    QList<QStringList> studentData;

    // 模拟学生数据：学号，姓名，性别，班级等
    studentData.append ({"2023414300229", "朱炳臻", "男", "2023 软件卓越 2 班", "计算机学院", "软件工程", "初修"});
    studentData.append ({"2023414300230", "冼广源", "男", "2023 软件卓越 2 班", "计算机学院", "软件工程", "初修"});
    studentData.append ({"2023414290102", "柴承源", "男", "2023 软件1班 (计科杨班)", "计算机学院", "软件工程", "初修"});
    studentData.append ({"2023414300110", "李嘉杰", "男", "2023 软件1班 (计科杨班)", "计算机学院", "软件工程", "初修"});
    studentData.append ({"2023414300225", "张周伟", "男", "2023 软件1班 (计科杨班)", "计算机学院", "软件工程", "初修"});
    return studentData;
}
...
{
    // 获取学生名单数据
    QString pdfPath = "学生名单.pdf";  // PDF 文件路径
    QList<QStringList> studentData = extractStudentDataFromPDF(pdfPath);

    // 设置表头
    ui->tableInfo->setColumnCount(7);  // 7列
    ui->tableInfo->setHorizontalHeaderLabels({"学号", "姓名", "性别", "行政班级", "院系", "专业", "修读性质"});

    // 设置行数
    ui->tableInfo->setRowCount(studentData.size());
    ...
    for (int row = 0; row < studentData.size(); ++row) {
        const QString name = studentData[row][1];              // 姓名在第2列
        if (name == QStringLiteral("柴承源")) {
            QFont boldFont = ui->tableInfo->font();
            boldFont.setBold(true);
            const QBrush redBrush(Qt::red);

            const int cols = ui->tableInfo->columnCount();
            for (int col = 0; col < cols; ++col) {
                QTableWidgetItem *it = ui->tableInfo->item(row, col);
                if (!it) {                                     // 保险：若该格未创建，先补一个项
                    it = new QTableWidgetItem;
                    ui->tableInfo->setItem(row, col, it);
                }
                it->setFont(boldFont);                         // 加粗
                it->setForeground(redBrush);                   // 文字变红（Qt5/6 用 setForeground）
                // 如需整行底色也变色，可再加一行：
...
{
    qDebug() << "Set Student List triggered!";

    // 你可以在这里调用其他函数来更新表格
    on_action_triggered();  // 调用之前定义的其他槽函数
}
